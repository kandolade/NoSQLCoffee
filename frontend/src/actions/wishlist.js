...
try {
    ...
    const {data}=await coffee.post('/api/products/wishlist',item,{
        headers:{
...
export const  getWishlist=()=>async (dispatch,getState)=>{
    try {
        const user = getState().user?.user;
        const {data}=await coffee.get('/api/products/wishlist',{
            headers:{
                Authorization: `Bearer ${user.token}` 
             }
        })
...
export const deleteItemFromWishlist=(id)=>async(dispatch,getState)=>{
    ...
    try {
        const user = getState().user?.user;
        const {data}=await coffee.delete(`/api/products/wishlist/${id}`,{
            headers:{
                Authorization: `Bearer ${user.token}` 
             }
        })
