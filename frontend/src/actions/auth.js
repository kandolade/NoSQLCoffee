...
export const siginUser = (email,password)=>async(dispatch,getState)=>{
   ...
   try {
       const {data}=await coffee.post('/api/users/signin',{email,password})
...
export const signupUser =(name,email,password)=>async dispatch =>{
   ...
   try {
       const {data}=await coffee.post('/api/users/signup',{name,email,password})
...
export const updateprofile=(userInfo)=>async (dispatch,getState)=>{
    ...
    try{
      ...
      const {data} = await coffee.put('/api/users/updateProfile',userInfo,{
        headers:{
